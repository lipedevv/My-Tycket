...
configure_nginx() {
    ...
    case "$ssl_type" in
        "letsencrypt")
            # Verificar se existe certificado para o backend
...
EOF
                ...
                echo -e "   ${YELLOW}⚠️ Certificado Let's Encrypt não encontrado para $BACKEND_DOMAIN${NC}"
                echo -e "   🔄 Usando certificado do frontend como fallback${NC}"
...
EOF
    ...
    case "$ssl_type" in
        "letsencrypt")
            # Verificar se existe certificado para o frontend
...
EOF
                ...
                echo -e "   ${RED}❌ Certificado Let's Encrypt não encontrado para $FRONTEND_DOMAIN${NC}"
                return 1
            fi
            ;;
        "selfsigned")
